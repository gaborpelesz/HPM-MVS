...
__device__ void PlaneHypothesisRefinement_checkerboard(const cudaTextureObject_t* images, const cudaTextureObject_t* depth_images, const Camera* cameras, float4* plane_hypothesis, float* depth, float* cost, curandState* rand_state, const float* view_weights, const float weight_norm, float4* prior_planes, unsigned int* plane_masks, float* restricted_cost, const int2 p, const PatchMatchParams params) {
    ...
    float depth_up, depth_down, depth_left, depth_right;
    float4 plane_hypothesis_up, plane_hypothesis_down, plane_hypothesis_left, plane_hypothesis_right;

    const int num_planes = 6;
    float depths[num_planes] = { depth_rand, *depth, depth_rand, *depth, depth_perturbed, depth_perturbed };
    float4 normals[num_planes] = { *plane_hypothesis, plane_hypothesis_rand, plane_hypothesis_rand, plane_hypothesis_perturbed, *plane_hypothesis, plane_hypothesis_perturbed };
...
__device__ void CheckerboardPropagation(const cudaTextureObject_t *images, const cudaTextureObject_t *depths, const Camera *cameras, float4 *plane_hypotheses, float *costs, curandState *rand_states, unsigned int *selected_views, float4 *prior_planes, unsigned int *plane_masks, const int2 p, const PatchMatchParams params, const int iter)
{
    ...
    //change 10 to 5 when multi scale
    ...
    // 0 -- left_up, 1 -- up_far, 2 -- right_up, 3 -- down_far, 4 -- right_down, 5 -- left_far, 6 -- left_down, 7 -- right_far
    bool flag[8] = { false };
...
#pragma omp parallel for
    ...
    const int positions[8] = { positions_tmp[0], positions_tmp[1] ,positions_tmp[2] ,positions_tmp[3] ,positions_tmp[4] ,positions_tmp[5] ,positions_tmp[6] ,positions_tmp[7] };
    ...
    int neighbor_positions[4] = { center - width * 5 , center + width * 5, center - 5, center + 5 };
    for (int i = 0; i < 4; ++i) {
        if (flag[2 * i + 1]) {
            for (int j = 0; j < params.num_images - 1; ++j) {
...
__global__ void JBU_cu(JBUParameters* jp, JBUTexObj* jt, float* depth, float4* normal, float4* normal_origin_cuda) {
    const int2 p = make_int2(blockIdx.x * blockDim.x + threadIdx.x, blockIdx.y * blockDim.y + threadIdx.y);
    ...
    for (int j = -num_neighbors; j <= num_neighbors; ++j) {
        ...
        for (int i = -num_neighbors; i <= num_neighbors; ++i) {
            ...
            const int s_center = r_y * jp[0].s_width + r_x;
            srcPix = tex2D<float>(jt[0].imgs[1], r_x + 0.5f, r_y + 0.5f);
            srcNorm = normal_origin_cuda[s_center];
            //printf("%f %f %f %f %f\n", srcNorm.x, srcNorm.y, srcNorm.z, srcNorm.w, srcPix);
            r_xs = p.x + i;
    ...
    depth[center] = c_total_val / normalizing_factor;
    normal[center] = n_total_val;
}
...
void JBU::CudaRun()
{
    ...
    JBU_cu << < grid_size_initrand, block_size_initrand >> > (jp_d, jt_d, depth_d, normal_d, normal_origin_cuda);
    cudaDeviceSynchronize();

    cudaMemcpy(depth_h, depth_d, sizeof(float) * rows * cols, cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();

    cudaMemcpy(normal_h, normal_d, sizeof(float4) * rows * cols, cudaMemcpyDeviceToHost);
